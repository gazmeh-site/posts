...
- **دو رابط کاربری:** خط فرمان (CLI) و رابط چت راست‌چین وب با Chainlit. هر دو از منطق مشترک `controller.next_command` استفاده می‌کنند.
...
# یا:
uv run python -m mirza
```

میرزا در طول کار، بسته به مرحله، ورودی‌های لازم را از تو می‌گیرد؛ از جمله حالت تولید (`mdfy` یا `auto`)، متن یا سرفصل‌ها، تأیید متن، تأیید مشخصات و مسیر، حالت تصویر و تأیید پرامپت‌های تصویری.

برای ویرایش دستی متن هم از `$EDITOR` استفاده می‌کند.
